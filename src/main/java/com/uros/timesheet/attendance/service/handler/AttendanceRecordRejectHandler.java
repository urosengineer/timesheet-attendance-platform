...
public class AttendanceRecordRejectHandler {
    ...
    public AttendanceRecordResponse handle(UUID id, UUID approverId, String reason) {
        ...
        try {
            ...
            boolean isPrivileged = approverRoles.contains("ADMIN") ||
                    approverRoles.contains("HR") ||
                    approverRoles.contains("MANAGER");

            if (record.getUser().getId().equals(approverId) && !isPrivileged) {
                throw new IllegalArgumentException(messageUtil.get("error.attendance.reject.self"));
            }

            if (!workflowEngineService.canTransition(
                    "AttendanceRecord", record.getStatus(), "REJECTED", approverRoles)) {
            ...
            timerSample.stop(meterRegistry.timer("attendance.reject.latency", "status", success ? "SUCCESS" : "FAIL"));
            meterRegistry.counter(
                    "attendance.reject.count",
